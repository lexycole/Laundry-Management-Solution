...
class PaymentMethodController extends Controller
{
    ...
    {   
        $input = $request->all();
        if($input['lang'] == 'en'){
            $data = PaymentMethod::where('status',1)->where('id','!=',1)->select('id','payment_mode','icon','status')->get();
        }else{
            $data = PaymentMethod::where('status',1)->where('id','!=',1)->select('id','payment_mode_ar as payment_mode','icon','status')->get();
        }
